...
function htmlNodeToWord(node, inherited = {}) {
    ...
    // Pre-rendered formula (see mathblock.docxMath): the marker carries the
    // OMML body as text content. It is generated by math-omml.js with all
    // dynamic text escaped, so it is inlined verbatim. Content that does not
    // look like our own OMML (a smuggled marker in stored HTML) is treated
    // as plain text instead of raw XML.
    const omml = tag === 'span' ? node.getAttribute('data-npad-omml') : null;
    if (omml) {
        const body = (node.textContent || '').trim();
        if (body.startsWith('<m:')) return `<m:oMath>${body}</m:oMath>`;
        return wordRun(node.textContent, inherited);
    }
...
function paragraphToWord(element, prefix = '', documentDirection = 'ltr') {
    // A display formula is its own centered paragraph of native math.
    if (element.matches?.('span[data-npad-omml="block"]')) {
        return `<w:p><w:pPr><w:jc w:val="center"/></w:pPr><m:oMathPara><m:oMath>${element.textContent || ''}</m:oMath></m:oMathPara></w:p>`;
    }
...
<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:m="http://schemas.openxmlformats.org/officeDocument/2006/math"><w:body>${htmlToWordBody(body, direction)}<w:sectPr><w:pgSz w:w="12240" w:h="15840"/><w:pgMar w:top="1440" w:right="1440" w:bottom="1440" w:left="1440"/></w:sectPr></w:body></w:document>`;
...
/**
 * Formulas inside an imported Word file have no HTML equivalent yet, so the
 * OMML is flattened to readable linear text: fractions become a/b, powers
 * a^2, radicals √(x), n-ary operators ∑_{i}^{n} e. Unknown constructs fall
 * back to their concatenated text, so content never disappears.
 */
function ommlToLinearText(node) {
    const local = node.localName;
    const kids = () => [...node.children].map(ommlToLinearText).join('');
    const byLocal = (name) => [...node.children].find((child) => child.localName === name);
    const part = (name) => {
        const found = byLocal(name);
        return found ? ommlToLinearText(found) : '';
    };
    if (local === 't' || local === 'r') return node.textContent;
    if (local === 'f') return `${part('num')}/${part('den')}`;
    if (local === 'sSup') return `${part('e')}^(${part('sup')})`;
    if (local === 'sSub') return `${part('e')}_${part('sub')}`;
    if (local === 'sSubSup') return `${part('e')}_${part('sub')}^(${part('sup')})`;
    if (local === 'rad') {
        const degree = byLocal('radPr')?.children.some((child) => child.localName === 'degHide')
            ? '' : part('deg');
        const radicand = part('e');
        return degree ? `${degree}√(${radicand})` : `√(${radicand})`;
    }
    if (local === 'nary') return naryToLinearText(node);
    if (local === 'naryPr' || local === 'chr' || local === 'subHide' || local === 'supHide') return '';
    if (local === 'limLow') return `${part('e')}(${part('lim')})`;
    if (local === 'limUpp') return `${part('e')}^(${part('lim')})`;
    if (local === 'acc') return part('e');
    if (local === 'd') return `(${kids().trim()})`;
    if (local === 'func') return kids();
    if (local === 'fName') return kids();
    return kids();
}

function naryToLinearText(nary) {
    const chrNode = [...nary.children].find((child) => child.localName === 'naryPr')
        && [...child.children].find((grand) => grand.localName === 'chr');
    const chr = (chrNode?.textContent || '∑').trim() || '∑';
    return `${chr}_{${part(nary, 'sub')}}^(${part(nary, 'sup')}) ${part(nary, 'e')}`.trim();
}

function part(node, name) {
    const found = [...node.children].find((child) => child.localName === name);
    return found ? ommlToLinearText(found) : '';
}
    ...
    for (const child of paragraph.children) {
        if (child.localName === 'r') content += docxRunToHtml(child);
            ...
            const text = [...child.children]
                .filter((item) => item.localName === 'oMath').map(ommlToLinearText).join(' ');
            if (text.trim()) content += escapeHtml(text);
        } else if (child.localName === 'oMath') {
            const text = ommlToLinearText(child);
            if (text.trim()) content += escapeHtml(text);
        }
    }
