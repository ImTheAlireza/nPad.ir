...
export default async function run(check, group) {
    ...
    for (const [key, lang, dir] of [['en', 'en', 'ltr'], ['fa', 'fa', 'rtl']]) {
        ...
        check('menus are keyboard/touch operable buttons', () => {
            ...
            assert.ok(imageToolbar, 'image block toolbar missing');
            assert.equal(imageToolbar.getAttribute('role'), 'toolbar');
            assert.equal(imageToolbar.hidden, true, 'image block toolbar should be contextual');
            assert.ok(imageToolbar.querySelectorAll('[data-image-action]').length >= 7,
                'image block toolbar is incomplete');
        });
